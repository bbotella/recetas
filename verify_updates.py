...
def verify_updates():
    ...
    recipes = conn.execute("SELECT id, title, description FROM recipes ORDER BY id").fetchall()
    ...
    for recipe in recipes:
        ...
        if desc_length > 300:  # Enhanced Spanish descriptions should be much longer
            ...
            print(f"⚠️  {recipe['title']} - {desc_length} characters (may need enhancement)")
    ...
    for translation in en_translations:
        ...
        if desc_length > 250:  # Enhanced English descriptions should be longer
            ...
            print(f"⚠️  {translation['title']} - {desc_length} characters (may need enhancement)")
...
def show_sample_updates():
    ...
    # Show English sample
    sample_en = conn.execute("SELECT * FROM recipe_translations WHERE recipe_id = 147 AND language = 'en'").fetchone()
    if sample_en:
        print(f"\n🇬🇧 English - {sample_en['title']}:")
        print(f"   {sample_en['description'][:200]}...")
    
    # Show Chinese sample
    sample_zh = conn.execute("SELECT * FROM recipe_translations WHERE recipe_id = 147 AND language = 'zh'").fetchone()
    if sample_zh:
