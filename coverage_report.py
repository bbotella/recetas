...
def generate_coverage_report():
    ...
    for file_path, data in coverage_report.items():
        ...
        for func in data["functions"]:
            if not func["name"].startswith("_"):  # Skip private functions
                file_functions += 1
                is_tested = any(
                    tested_func in func["name"] for tested_func in tested_functions
                )
        ...
        file_coverage = (
            (file_tested / file_functions * 100) if file_functions > 0 else 0
        )
    ...
    print("\n" + "=" * 50)
    print(
        f"Overall Coverage: {tested_count}/{total_functions} ({overall_coverage:.1f}%)"
    )
...
def main():
    ...
    print(
        f"✓ pytest configuration: {'pytest.ini' if os.path.exists('pytest.ini') else 'Missing'}"
    )
    print(
        f"✓ Test fixtures: {'conftest.py' if os.path.exists('tests/conftest.py') else 'Missing'}"
    )
    print(
        f"✓ Requirements: {'requirements.txt' if os.path.exists('requirements.txt') else 'Missing'}"
    )
