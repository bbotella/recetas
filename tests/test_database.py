...
from database import (
    init_database, get_db_connection, get_all_recipes, get_recipe_by_id,
    search_recipes, get_categories, get_recipe_translation, 
    get_recipe_with_translation, get_all_recipes_with_translation,
    search_recipes_with_translation, save_recipe_translation
)
...
class TestDatabaseInitialization:
    ...
    def test_init_database_creates_tables(self, app):
        ...
        with app.app_context():
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='recipe_translations'")
            assert cursor.fetchone() is not None
    ...
    def test_init_database_creates_correct_schema(self, app):
        ...
        with app.app_context():
            ...
            expected_columns = ['id', 'title', 'description', 'ingredients', 'instructions', 'category', 'filename', 'created_at']
            ...
            columns = [column[1] for column in cursor.fetchall()]
            expected_columns = ['id', 'recipe_id', 'language', 'title', 'description', 'ingredients', 'instructions', 'category', 'created_at']
            for col in expected_columns:
                assert col in columns
...
class TestTranslationOperations:
    ...
    def test_get_recipe_with_translation_fallback(self, app):
        ...
        with app.app_context():
            recipe = get_recipe_with_translation(2, 'en')  # Recipe 2 has no English translation
            assert recipe is not None
