...
from database import (
    search_recipes, search_recipes_with_translation, 
    get_categories, get_all_recipes, get_all_recipes_with_translation
)
...
class TestBasicSearch:
    ...
    def test_search_by_description(self, app):
        ...
        with app.app_context():
            ...
            assert len(results) >= 1
            assert any('Test description 1' in recipe['description'] for recipe in results)
...
class TestSearchWithTranslations:
    ...
    def test_search_with_translation_and_category(self, app):
        ...
        with app.app_context():
            results = search_recipes_with_translation('Test', category='Desserts', language='en')
            assert len(results) >= 1
    ...
    def test_search_translated_categories(self, app):
        ...
        with app.app_context():
            ...
            results = search_recipes_with_translation('', category='Desserts', language='en')
            assert len(results) >= 1
            assert any('Test Recipe 1 EN' in recipe['title'] for recipe in results)
