...
def translate_to_english(text, context):
    ...
    if context == "ingredients":
        ...
        if "Descubre la magia culinaria" in text:
            translated = re.sub(
                r"Descubre la magia culinaria de ([^,]+)",
                r"Discover the culinary magic of \1",
                translated,
            )
...
def translate_to_catalan(text, context):
    ...
    if context == "ingredients":
        ...
        if "Descubre la magia culinaria" in text:
            translated = re.sub(
                r"Descubre la magia culinaria de ([^,]+)",
                r"Descobreix la màgia culinària de \1",
                translated,
            )
...
def translate_to_basque(text, context):
    ...
    if context == "ingredients":
        ...
        if "Descubre la magia culinaria" in text:
            translated = re.sub(
                r"Descubre la magia culinaria de ([^,]+)",
                r"Deskubritu \1ren magia kulinarioa",
                translated,
            )
...
def fix_all_translations():
    ...
    for translation in problematic:
        ...
        # Corregir cada campo
        fixed_description = generate_proper_translation(
            description, language, "description"
        )
        fixed_ingredients = generate_proper_translation(
            ingredients, language, "ingredients"
        )
        fixed_instructions = generate_proper_translation(
